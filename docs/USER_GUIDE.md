...
ValgACE поддерживает команды для управления соединением с устройством:

#### `ACE_CONNECT`
Команда для подключения к устройству ACE, если оно было отключено.
...
#### `ACE_DISCONNECT`
Команда для принудительного отключения от устройства ACE.
...
Команда для проверки текущего статуса соединения с устройством.
...
Команда для проверки статуса внешнего датчика филамента (если он настроен в конфигурации).
...
Эти команды полезны для диагностики проблем с подключением и мониторинга состояния устройства.
