...
Включает альтернативный алгоритм парковки с использованием датчика филамента.
...
**Как работает:**
- При включении начинается подача филамента
- Когда датчик обнаруживает филамент, переключается на традиционный алгоритм
- Полезно для более надежной парковки в некоторых конфигурациях

**Требования:**
- Должен быть настроен внешний датчик филамента через параметр `filament_sensor`

**Алгоритмы парковки:**
- При включении `aggressive_parking` используются два алгоритма:
  - **Sensor-based parking**: Использует внешний датчик филамента для определения момента парковки
  - **Distance-based parking**: Используется при отсутствии датчика филамента, подает филамент на заданное расстояние

**Дополнительные параметры:**
- `max_parking_distance`: Максимальное расстояние (в мм) для подачи филамента при агрессивной парковке
- `parking_speed`: Скорость подачи филамента (в мм/с) при агрессивной парковке
- `extended_park_time`: Дополнительное время (в секундах) к таймауту парковки на основе датчика
- `max_parking_timeout`: Максимальное время ожидания (в секундах) завершения парковки при смене инструмента

---

### `max_parking_distance`

Максимальное расстояние (в мм) для подачи филамента при агрессивной парковке.
...
**Примечание:** Если филамент не обнаружен датчиком на этом расстоянии, парковка будет прервана с ошибкой.

---

### `parking_speed`

Скорость подачи филамента (в мм/с) при агрессивной парковке.
...
**Рекомендации:**
- Меньшая скорость = более точное определение датчиком
- Большая скорость = быстрее парковка, но менее точно

---

### `extended_park_time`

Дополнительное время (в секундах) к таймауту парковки на основе датчика.
...
**Примечание:** Это время добавляется к расчетному таймауту парковки для обеспечения надежного завершения процесса.

---

### `max_parking_timeout`

Максимальное время ожидания (в секундах) завершения парковки при смене инструмента.
...
**Примечание:** Если парковка не завершена за это время, операция будет прервана с ошибкой.
...
Время debounce (в секундах) для подтверждения статуса 'empty' при автоматическом мониторинге.
...
**Как работает:**
- При обнаружении статуса 'empty' запускается debounce таймер
- Статус должен оставаться 'empty' в течение указанного времени
- Предотвращает ложные срабатывания при временных изменениях статуса

**Рекомендации:**
- `1.0-2.0` - для стабильной работы
- `3.0-5.0` - для ненадежных датчиков или проблемных слотов

---

### `infinity_spool_pause_on_no_sensor`

Пауза печати при отсутствии датчика филамента во время infinity spool.
...
**Как работает:**
- **При `True`**: Если нет датчика филамента, печать ставится на паузу при обнаружении empty статуса, позволяя пользователю вручную подтвердить смену
- **При `False`**: Если нет датчика филамента, смена слота происходит немедленно после подтверждения empty статуса (требует уверенности в надежности)

**Рекомендации:**
- `True` - безопасный режим, рекомендуется для большинства пользователей
- `False` - для полностью автоматизированной печати без вмешательства

---

### Infinity Spool Auto-trigger система

При включенном `infinity_spool_mode` работает автоматический мониторинг статуса активного слота во время печати:

**Алгоритм работы:**

1. **Обнаружение empty статуса:**
   - Мониторинг статуса активного слота каждые 0.5 секунд
   - При обнаружении 'empty' запускается debounce таймер

2. **Подтверждение empty статуса:**
   - После истечения `infinity_spool_debounce` секунд статус проверяется повторно
   - Если статус остается 'empty', запускается процедура смены

3. **Смена слота:**
   - **С датчиком филамента**: Мониторит датчик до срабатывания, затем вызывает `ACE_INFINITY_SPOOL`
   - **Без датчика**:
     - При `infinity_spool_pause_on_no_sensor=True` - пауза печати
     - При `infinity_spool_pause_on_no_sensor=False` - немедленный вызов `ACE_INFINITY_SPOOL`

**Пример конфигурации для полностью автоматической печати:**
```ini
[ace]
serial: /dev/serial/by-id/usb-ANYCUBIC_ACE_1-if00
baud: 115200
...
infinity_spool_mode: True
infinity_spool_debounce: 2.0
infinity_spool_pause_on_no_sensor: False  # Автоматическая смена без паузы

# Внешний датчик филамента (рекомендуется)
...
Имя внешнего датчика филамента для интеграции с ACE модулем.

**Тип:** строка (имя датчика)
...
**Что делает:**
- Интегрирует внешний датчик филамента с ACE модулем
- Позволяет проверять статус датчика через команду `ACE_CHECK_FILAMENT_SENSOR`
- Включает информацию о датчике в общий статус устройства (через `ACE_STATUS` и Moonraker API)
- Позволяет использовать датчик в макросах и автоматизации

**Примечание:** Имя должно соответствовать имени датчика, определенного в конфигурации Klipper (например, `[filament_switch_sensor my_filament_sensor]`).

---

## Поля статуса

Модуль ACE возвращает дополнительные поля статуса через метод `get_status`:

### `feed_assist_slot`
- Индекс слота с активным feed assist (-1 если выключен)
- Показывает какой слот в данный момент использует ассистент подачи

### `filament_sensor`
- Статус внешнего датчика филамента (если настроен)
- Включает информацию о наличии филамента и статусе датчика

### `slot_mapping`
- Информация об отображении индексов в слоты
- Показывает текущее соответствие индексов Klipper (T0-T3) физическим слотам устройства
...
Имя макроса для вызова при потере связи во время печати.
...
**Как работает:**
- При потере связи с устройством ACE во время печати вызывается указанный макрос
- Это позволяет безопасно остановить печать и предотвратить проблемы
- Можно указать пользовательский макрос паузы, если он отличается от стандартного

**Пример пользовательского макроса:**
```ini
set_pause_macro_name: MY_CUSTOM_PAUSE
```

**Примечание:** Убедитесь, что указанный макрос существует в конфигурации Klipper.
...
### Конфигурация с агрессивной парковкой
...
# Внешний датчик филамента (обязательно для агрессивной парковки)
filament_sensor: my_filament_sensor
...
**Примечание:** Для использования агрессивной парковки необходимо настроить внешний датчик филамента в Klipper:

```ini
[filament_switch_sensor my_filament_sensor]
switch_pin: <ваш_пин_датчика>
pause_on_runout: False  # Управление через ACE
```
...
*Дата последнего обновления: 2026-03*
