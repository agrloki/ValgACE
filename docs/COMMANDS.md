...
5. [Маппинг слотов](#маппинг-слотов)
6. [Управление индексами](#управление-индексами)
7. [Связь и соединение](#связь-и-соединение)
8. [Режим бесконечной катушки](#режим-бесконечной-катушки)
9. [Отладочные команды](#отладочные-команды)
10. [Алиасы команд](#алиасы-команд)
...
- Индекс слота с активным feed assist (`feed_assist_slot`)
- Статус внешнего датчика филамента (если настроен)
- Отображение индексов в слоты (`slot_mapping`)
...
Получить справку по всем доступным командам ACE.
...
**Что делает:**
- Выводит список всех доступных ACE команд с кратким описанием
- Группирует команды по категориям для удобства
...
**Примечание:** Полезно для быстрого ознакомления с доступными командами без обращения к документации.
...
Команды для управления отображением индексов Klipper (T0-T3) в физические слоты устройства.
...
**Что делает:**
- Выводит текущее отображение индексов Klipper (T0-T3) в физические слоты устройства
- Полезно для диагностики нестандартных конфигураций
...
**Примечание:** По умолчанию используется прямое отображение (0→0, 1→1, 2→2, 3→3).
...
Получить текущее значение индекса инструмента.
...
**Что делает:**
- Выводит текущее значение переменной `ace_current_index`
- Полезно для диагностики и проверки текущего состояния
...
**Примечание:** Команда показывает индекс, который сохраняется в переменных Klipper и используется для отслеживания текущего активного инструмента.

---

### `ACE_SET_CURRENT_INDEX`

Установить текущее значение индекса инструмента.

**Синтаксис:**
```gcode
ACE_SET_CURRENT_INDEX INDEX=<значение>
```

**Параметры:**
- `INDEX` (обязательный) - Значение индекса (-1 до 3):
  - `-1` - Нет активного инструмента (филамент выгружен)
  - `0-3` - Номер активного инструмента

**Что делает:**
- Устанавливает произвольное значение индекса инструмента
- Полезно при возникновении ошибок, когда правильный индекс не был записан во время смены филамента
- Обновляет переменную `ace_current_index` в сохраняемых переменных Klipper

**Примеры:**
```gcode
ACE_SET_CURRENT_INDEX INDEX=0    # Установить текущий инструмент как 0
ACE_SET_CURRENT_INDEX INDEX=-1   # Установить как "нет активного инструмента"
```
...
**Примечание:** Используйте эту команду с осторожностью, только для восстановления после ошибок или в специфических сценариях.
...
**Параметры:**
- `INDEX` (обязательный) - Индекс из Klipper (T0-T3), значение от 0 до 3
- `SLOT` (обязательный) - Физический слот устройства, значение от 0 до 3

**Что делает:**
- Устанавливает отображение индекса Klipper в физический слот устройства
- Полезно при нестандартном подключении катушек
...
# Нестандартное отображение: T0 использует физический слот 2
ACE_SET_SLOTMAPPING INDEX=0 SLOT=2

# Поменять местами T0 и T1
...
- Настройка сохраняется до перезагрузки Klipper
- Для постоянных изменений используйте конфигурационный файл
...
**Что делает:**
- Сбрасывает отображение слотов в дефолтные значения (0→0, 1→1, 2→2, 3→3)
- Полезно для возврата к стандартной конфигурации после экспериментов
...
**Примечание:** После сброса все индексы T0-T3 будут соответствовать физическим слотам 0-3.
...
**Что делает:**
- Сбрасывает флаги ошибок соединения
- Пытается переподключиться к ACE устройству
- Полезно после потери связи или ошибок подключения
...
**Когда использовать:**
- После потери связи с устройством
- При ошибках подключения
- Для восстановления соединения без перезапуска Klipper

**Примечание:** Команда очищает внутренние флаги ошибок и инициирует новое подключение к устройству.
...
- При наличии флага потери связи выводит дополнительную информацию о попытках переподключения
...
# Во время печати:
# 1. Слот 0 заканчивается -> статус меняется на 'empty'
# 2. Debounce таймер 2 секунды
# 3. Статус подтверждён -> автоматический вызов ACE_INFINITY_SPOOL
# 4. Переход на следующий слот согласно порядку
```
...
# Установить нестандартный маппинг (T0 -> физический слот 2)
ACE_SET_SLOTMAPPING INDEX=0 SLOT=2
...
# Переподключиться при проблемах
ACE_RECONNECT

# Полное отключение и повторное подключение
ACE_DISCONNECT
ACE_CONNECT
```
