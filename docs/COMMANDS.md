...
**Что делает:**
- Переключается на следующий готовый слот автоматически
- Не выполняет откат (филамент уже закончился)
- Используется счетчик для отслеживания слотов
...
```gcode
# В макросах слайсера при обнаружении окончания филамента
ACE_INFINITY_SPOOL
```

**Процесс:**
1. Проверка режима infinity_spool
2. Определение следующего слота
3. Проверка готовности слота
4. Выполнение макроса `_ACE_PRE_INFINITYSPOOL`
5. Парковка нового филамента
6. Выполнение макроса `_ACE_POST_INFINITYSPOOL`

**Ограничения:**
- Максимум 4 слота (0-3)
- Работает только при включенном режиме
- Требует готовности следующего слота
