...
class ValgAce:
    ...
    def _serial_lock(self):
        ...
        with self.lock:  # Исправление: используем lock как объект Lock()
            try:
                yield
            ...
            try:
                ...
                if not self._queue.empty():
                    ...
                    if task:
                        ...
                        # Добавляем защиту от ошибок отправки
                        try:
                            ...
                            self.logger.error(f"Send request error: {str(e)}")
    ...
    def _reconnect(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Reconnect error: {str(e)}", exc_info=True)
            return False
    ...
    def _disconnect(self):
        ...
        if hasattr(self, '_writer_thread') and self._writer_thread != current_thread:
            try:
                ...
                self.logger.error(f"Writer thread join error: {str(e)}", exc_info=True)
        ...
        if hasattr(self, '_reader_thread') and self._reader_thread != current_thread:
            try:
                ...
                self.logger.error(f"Reader thread join error: {str(e)}", exc_info=True)
                
        if hasattr(self, 'main_timer'):
            try:
                self.reactor.unregister_timer(self.main_timer)
            except Exception as e:
                self.logger.error(f"Timer unregister error: {str(e)}", exc_info=True)
        ...
        with self._serial_lock():
            try:
                ...
                try:
                    ...
                    self.logger.error(f"JSON encoding error: {str(e)}", exc_info=True)
                    return False
                ...
                try:
                    ...
                    self.logger.error(f"Serial write error during send: {str(e)}", exc_info=True)
                    self._reset_connection()
                    return False
    ...
    def _process_message(self, msg: bytes):
        ...
        try:
            ...
            if len(msg) < 4 + payload_len + 3:
                self.logger.warning(f"Incomplete message received: {msg}")
                return
    ...
    def send_request(self, request: Dict[str, Any], callback: Callable):
        ...
        if self._queue.qsize() >= self._max_queue_size:
            ...
            # Вместо полной очистки, лучше реализовать механизм graceful degradation
            try:
