...
class ValgAce:
    ...
    def _writer_loop(self):
        """Цикл записи с использованием reactor для фонового потока"""
                ...
                if not self._queue.empty():
                    ...
                    if task:
                        ...
                        if not self._send_request(request):
                            ...
                            self.dwell(0.1, lambda: None)
                            continue
                self.dwell(0.05, lambda: None)
    ...
    def _reader_loop(self):
        """Цикл чтения с использованием reactor для фонового потока"""
    ...
    def dwell(self, delay: float = 1.0, callback: Optional[Callable] = None):
        """Асинхронная пауза с использованием реактора и необязательным callback"""
        if delay <= 0:
            if callback:
        ...
        def timer_handler(event_time):
            if callback:
                self._main_queue.put(callback)
            return self.reactor.NEVER  # Однократный таймер
